...
// friend firing on the dragon (see buildAllySquad), the fight still has
...
// constant so they stay lined up.
const WEAK_OFFSET_Y = 20;
// How often the next friend in the squad takes a shot, and how long their
// shot takes to reach the dragon. This is the only source of damage in the
...
export default class BossScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.rectangle(W / 2, GROUND_Y + 40, W, 80, PALETTE.ground);
  ...
  // falls. Reuses her own face photo (playerConfig.facePhoto).
  buildJailCell() {
  ...
  // allyVolley) -- the only thing that damages it. There's no separate
  // controllable player character in this scene: everyone who's already
  // been rescued is already standing here, so a distinct extra figure
  // alongside them (which used to double as Akansha's own likeness,
  // despite her being the one in the cage) was redundant.
  buildAllySquad() {
  ...
  // actually is. A plain yoyo tween paused mid-flight and resumed later
  // snaps back onto its own original start/end interpolation instead of
  // continuing from the paused spot, which looked like teleporting.
  startPatrolFrom(x) {
  ...
  // Shared by nothing else now except the squad's automated volley (see
  // allyVolley) -- each hit only chips at a fine-grained counter, and the
  // HUD's displayed HP pip only drops every 4-5 hits, so 19 friends is
  // what actually finishes this fight, not a single lucky shot.
  registerHit() {
    if (this.finished) return;
  ...
  update() {
    if (this.finished || !this.dragonGroup) return;
    // Face whichever way it's actually moving -- a plain frame-to-frame
    // delta on x covers the whole patrol without needing separate facing
    // logic.
